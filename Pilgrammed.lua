...
local Functions = {}; do
	...
	function Functions:GetBuild(Player)
		...
		Insert(Accessories, tostring(Target.CharacterStats.Armor.Value))
		Insert(Accessories, tostring(Target.CharacterStats.Helmet.Value))
		task.wait()
...
local CallbackLists = {}; do
	...
	CallbackLists["Player%%Deposit Amount"] = function(Value)
	    Configuration[1]["Economy"].Bank = tonumber(Value)
	end
	
	CallbackLists["Player%%Edit Should Deposit"] = function(Value)
	    Configuration[1]["Economy"].BankDeposit = Value
	end
	
	CallbackLists["Player%%Deposit"] = function(IsDeposit, Value)
	    local Bank = LocalPlayer.PlayerStats.Bank.Value
	    local Cash = LocalPlayer.PlayerStats.Gold.Value
	    local DesiredAmount = Value
	
	    local SourceAmount = IsDeposit and Cash or Bank
	    if SourceAmount <= 0 then return end
	
	    local Fraction = DesiredAmount / SourceAmount
	    Fraction = Clamp(Fraction, 0, 1)
	
	    ReplicatedStorage.Remotes.Bank:InvokeServer(IsDeposit, Fraction)
	end
	
	CallbackLists["Player%%Withdrawal"] = function(Value)
...
local Window = WindUI:CreateWindow{Title = "Milkhook", Folder = "Milkhook", Size = DimOffset(600, 400), MinSize = Vec2(560, 350), MaxSize = Vec2(850, 560), HideSearchBar = false, User = {Enabled = true, Anonymous = false}} do
	...
	local Section = Window:Tab{Title = "Combat", Icon = "rbxassetid://110987169760162"} do
		...
		Section:Paragraph{Title = "Importance", Desc = "Method was absolutely killed and patched by the devs so this is like partially god mode 🕊🥀"}
	...
	local Section = Window:Tab{Title = "Player", Icon = "rbxassetid://125020872044147"} do
		...
		Section:Section{Title = "Economy"}
		Section:Button{Title = "Action", Desc = "Withdrawl/Deposit you're designated value below.", Callback = function()
			GetCallback("Player%%Deposit")(Configuration[1]["Economy"].BankDeposit, Configuration[1]["Economy"].Bank)
		end}
		Section:Toggle{Title = "Deposit", Type = "Toggle", Callback = GetCallback("Player%%Edit Should Deposit")}
		Section:Input{Title = "Amount", Type = "Input", Placeholder = "Number", Callback = GetCallback("Player%%Deposit Amount")}

		Section:Section{Title = "Teleport"}
		Section:Button{Title = "Action", Desc = "Set you're location to the designated npc.", Callback = GetCallback("Player%%NPCs Action")}
		Section:Dropdown{Title = "NPCs", Flag = "Teleport_NPCs", Desc = "Select the NPCs used on top.", Values = Configuration[4], Value = "Abraska", Callback = GetCallback("Player%%NPCs")}
		...
		Section:Button{Title = "Pitfall", Desc = "Set you're location to pitfall.", Callback = GetCallback("Player%%Pitfall Action")}
	end
